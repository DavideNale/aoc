...
pub fn process(_input: &str) -> miette::Result<String> {
    ...
    let mut result: Vec<Vec<usize>> = updates
        .into_iter()
        .filter(|vec| !is_safe(vec, &rules))
        .collect();

    let sorted_results: Vec<&mut Vec<usize>> = result
        .iter_mut()
        .map(|update| {
            update.sort_by(|a, b| {
                if orders.get(a).is_some_and(|page| page.contains(b)) {
                    Ordering::Less
                } else {
                    Ordering::Greater
                }
